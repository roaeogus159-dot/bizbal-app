// ② 세부 수정: 점/브러시/같은색 선택, 다중선택, 색교체, undo/redo, 초기화, 돋보기, 전문가 강조
  ...
  return (
    <div className="split">
      <div className="preview-area">
        <PreviewCanvas editable onCellTap={onCellTap} onBrushCells={onBrushCells} />
        {s.paintMode === 'expert' && (
      ...
      <div className="controls">
        <div className="card">
          ...
          <p className="muted hint">
            ...
            {p.tool === 'brush' && '드래그한 칸이 모두 선택됩니다. 두 손가락으로 화면 이동.'}
          ...
          <div className="edit-actions">
            ...
            <button
              ...
              disabled={p.undoStack.length === 0 && p.redoStack.length === 0}
              onClick={() => {
                if (window.confirm('수동 수정을 모두 취소하고 자동 변환 상태로 되돌릴까요?')) p.resetEdits()
              }}
            >
              ⟲ 초기화
            </button>
        ...
        <div className="card">
          <div className="replace-row">
            ...
            {currentColor !== null && palette[currentColor] && (
              <button
                className="suggest-chip"
                onClick={() => applyColor(currentColor)}
                disabled={p.selection.size === 0}
                title="스포이드로 지정한 현재 색을 선택 칸에 적용"
              >
                <BeadSwatch color={palette[currentColor]} size={24} />
                현재 색 적용
              </button>
            )}
            <button
